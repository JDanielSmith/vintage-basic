...
public sealed class TokenizerTests
{
	...
	public void LetStatementTokenizeTest()
	{
		...
		Assert.Collection(tokens,
			...
			t => { Assert.IsType<VarNameToken>(t.Value); Assert.Equal("a", ((VarNameToken)t.Value).Name); Assert.Null(((VarNameToken)t.Value).TypeSuffix); Assert.Equal(new SourcePosition(20, 5), t.Position); },
