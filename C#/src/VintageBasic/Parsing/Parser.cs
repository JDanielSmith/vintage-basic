...
sealed class Parser
{
    ...
    private Tagged<Token>? Peek(int offset = 0) => (_currentTokenIndex + offset < _tokens.Count) ? _tokens[_currentTokenIndex + offset] : null;
    private Token? PeekToken(int offset = 0) => Peek(offset)?.Value;
    private SourcePosition CurrentSourcePosition() => Peek()?.Position ?? new SourcePosition(_lineNumber, _tokens.LastOrDefault()?.Position.Column + 1 ?? (_tokens.FirstOrDefault()?.Position.Column ?? 1));

    private Tagged<Token> ConsumeToken()
    {
    ...
    private Tagged<T> ConsumeToken<T>(string? expectedText = null) where T : Token
    {
    ...
    private bool TryConsumeKeyword(KeywordType kw, out Tagged<KeywordToken>? consumedToken)
    {
        var current = Peek();
    ...
    private bool TryConsumeSpecificSymbol(string symbol, out Tagged<Token>? consumedToken)
    {
        var current = Peek();
    ...
    // --- Statement Parsers ---
    private Tagged<Statement>? TryParseStatement()
    {
    ...
    private PrintStmt TryParsePrintStatementContents()
    {
    ...
    private DimStmt TryParseDimStatementContents()
    {
    ...
    private NextStmt TryParseNextStatementContents()
    {
    ...
    private ReadStmt TryParseReadStatementContents()
    {
    ...
    private DataStmt TryParseDataStatementContents()
    {
    ...
    private InputStmt TryParseInputStatementContents()
    {
        string? prompt = null;
    ...
    private DefFnStmt TryParseDefFnStatementContents() 
    {
    ...
    private RestoreStmt TryParseRestoreStatementContents()
    {
        int? label = null;
    ...
    private Statement TryParseOnGotoOrGosubStatementContents()
    {
    ...
    private Tagged<Expr> TryParseExpression(int minPrecedence = 0) 
    {
    ...
    private Tagged<Expr> TryParseUnaryExpression()
    {
    ...
    private Tagged<Expr> TryParsePowerExpression()
    {
    ...
    private Tagged<Expr> TryParseAtom()
    {
    ...
    private Tagged<Expr> TryParseBuiltinFunctionCall()
    {
    ...
    private Tagged<Expr> TryParseUserFunctionCall() 
    {
    ...
    private static int GetOperatorPrecedenceForNot() => 2; 

    private static int GetOperatorPrecedence(BinOp op, bool isUnary = false)
    {
    ...
    private static bool IsRightAssociative(BinOp op) => op == BinOp.PowOp;
}
